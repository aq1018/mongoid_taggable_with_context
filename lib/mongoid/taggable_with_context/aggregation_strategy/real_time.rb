  ...
  module RealTime
    ...
    module ClassMethods
      ...
      def aggregation_database_collection_for(context)
        @aggregation_database_collection ||= Moped::Collection.new(self.collection.database, aggregation_collection_for(context))
      end
      ...
      def tags_with_weight_for(context, conditions={})
        aggregation_database_collection_for(context).find({:value => {"$gt" => 0 }}).sort(_id: 1).to_a.map{ |t| [t["_id"], t["value"]] }
      end
